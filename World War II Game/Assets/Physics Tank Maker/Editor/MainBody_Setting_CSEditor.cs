...
namespace ChobiAssets.PTM
{
	...
	public class MainBody_Setting_CSEditor : Editor
	{
		...
		void Set_Inspector ()
		{
			...
			Mass_Center_ZeroProp.boolValue = EditorGUILayout.Toggle ("Center of Mass to Zero", Mass_Center_ZeroProp.boolValue);
			EditorGUILayout.Space ();
		...
		void Create ()
		{
			PrefabUtility.DisconnectPrefabInstance (thisGameObject);
			...
			for (int i = 0; i < oldMeshColliders.Length; i++) {
				DestroyImmediate(oldMeshColliders[i]);
			}
			for (int i = 0; i < Colliders_NumProp.intValue; i++) {
			...
			// Layer settings.
			thisGameObject.layer = 11; // Main_Body ( ignore collision with wheels).
		}
