...
namespace ChobiAssets.PTM
{
	...
	public class Create_TrackBelt_CSEditor : Editor
	{
		...
		void Create_Reinforce (string direction, int number)
		{
			...
			// Set Layer
			reinforceObject.layer = 10; // Ignore All.
		}
		...
		void Create_Prefab ()
		{
			...
			PrefabUtility.CreatePrefab ("Assets/Physics Tank Maker/User/" + "Static_Track " + DateTime.Now.ToString ("yyMMdd_HHmmss") + ".prefab", newParentObject);
		...
		void Set_Parent_Script (Static_Track_Parent_CS parentScript)
		{
			...
			if (driveControlScript) {
				parentScript.Stored_Torque = driveControlScript.Torque * 1.25f; // To increase the "Torque" later.
				parentScript.Stored_Turn_Brake_Drag = driveControlScript.Turn_Brake_Drag * 3.0f; // To increase the "Turn_Brake_Drag" later.
			}
