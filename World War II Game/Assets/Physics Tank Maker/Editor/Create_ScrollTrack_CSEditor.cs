...
namespace ChobiAssets.PTM
{
	...
	public class Create_ScrollTrack_CSEditor : Editor
	{
		...
		void Create ()
		{
			PrefabUtility.DisconnectPrefabInstance (thisTransform.gameObject); // Break prefab connection.
		...
		void Create_SupportWheelsLisT ()
		{
			...
			foreach (Create_SupportWheel_CS supportWheelScript in supportWheelScripts) {
				...
				foreach (MeshFilter tempFilter in tempFilters) {
					if (tempFilter.gameObject.layer == 9 && tempFilter.transform.localEulerAngles.z == 0.0f && tempFilter.sharedMesh) { // Layer 9 == Wheel, && Left.
