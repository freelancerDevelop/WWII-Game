...
namespace ChobiAssets.PTM
{
	...
	public class Static_Track_Piece_CSEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			if (isPrepared) {
				...
				if (GUI.changed || Event.current.commandName == "UndoRedoPerformed") {
					PrefabUtility.DisconnectPrefabInstance (Selection.activeObject);
					Copy_To_Opposite (Selection.activeGameObject.GetComponent <Static_Track_Piece_CS>());
		...
		void Set_Inspector ()
		{
			...
			switch (TypeProp.intValue) {
				...
				case 1: // Anchor
					if (GUILayout.Button("Set Closest DrivingWheel", GUILayout.Width(200))) {
						if (Set_Closest_DrivingWheel(Selection.activeGameObject.GetComponent <Static_Track_Piece_CS>(), Mathf.Infinity) == false) {
							Debug.LogWarning("DrivingWheel cannot be found in this tank.");
						}
					}
					if (GUILayout.Button("Set Closest SwingBall", GUILayout.Width(200))) {
						if (Set_Closest_SwingBall(Selection.activeGameObject.GetComponent <Static_Track_Piece_CS>(), Mathf.Infinity) == false) {
			...
			Front_ScriptProp.objectReferenceValue = EditorGUILayout.ObjectField ("Front Script", Front_ScriptProp.objectReferenceValue, typeof(Static_Track_CS), true);
			Rear_ScriptProp.objectReferenceValue = EditorGUILayout.ObjectField ("Rear Script", Rear_ScriptProp.objectReferenceValue, typeof(Static_Track_CS), true);
		...
		{
			Transform	bodyTransform = Selection.activeGameObject.transform.parent.parent;
		...
		void Update_Scripts_Values()
		{
			...
			for (int i = 0; i < pieceScripts.Length; i++) {
				...
				pieceScripts[i].Pieces_Count = (pieceScripts.Length - 1) / 2;
			}
		}
