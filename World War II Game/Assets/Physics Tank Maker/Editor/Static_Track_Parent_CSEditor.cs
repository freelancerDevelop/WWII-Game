...
namespace ChobiAssets.PTM
{
	...
	public class Static_Track_Parent_CSEditor : Editor
	{
		...
		SerializedProperty Stored_Turn_Brake_DragProp;
		...
		void  OnEnable ()
		{
			...
			Stored_Turn_Brake_DragProp = serializedObject.FindProperty ("Stored_Turn_Brake_Drag");
		}
		...
		void  Set_Inspector ()
		{
			...
			EditorGUILayout.Space ();
			EditorGUILayout.Space ();
			EditorGUILayout.HelpBox ("Update scripts of the track pieces from version 2.5 to 3.0.", MessageType.None, true);
			if (GUILayout.Button ("Update Track Pieces", GUILayout.Width (200))) {
				Update_Track_Pieces ();
			}
		...
		{
			PrefabUtility.DisconnectPrefabInstance (Selection.activeGameObject.transform);
		...
		void Set_Other_Components_Automatically ()
		{
			...
			// "Drive_Control_CS" (Increase the "Torque" and "Turn_Brake_Drag", and disable the "Fix_Useless_Rotaion".)
			Drive_Control_CS driveControlScript = bodyTransform.GetComponent <Drive_Control_CS>();
			if (driveControlScript.Torque < Stored_TorqueProp.floatValue) {
				driveControlScript.Torque = Stored_TorqueProp.floatValue;
			}
			if (driveControlScript.Turn_Brake_Drag < Stored_Turn_Brake_DragProp.floatValue) {
				driveControlScript.Turn_Brake_Drag = Stored_Turn_Brake_DragProp.floatValue;
			}
		...
		void Set_Types_With_RoadWheels (float radius)
		{
			...
			PrefabUtility.DisconnectPrefabInstance (thisTransform);
			int count = 0;
		...
		void Update_Track_Pieces ()
		{
			...
			if (oldParentScript) {
				...
				newParentScript.Stored_Torque = oldParentScript.Stored_Torque;
				newParentScript.Stored_Turn_Brake_Drag = oldParentScript.Stored_Turn_Brake_Drag;
			...
			// Done.
			Debug.Log("The track pieces have been updated.");
		}
