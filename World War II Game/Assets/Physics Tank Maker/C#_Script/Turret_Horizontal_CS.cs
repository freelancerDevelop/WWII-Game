		...
		*/
		...
		void Auto_Turn ()
		{
			...
			if (isTracking) { // Tracking the target.
				...
				if (aimingScript.Target_Rigidbody && aimingScript.Use_Auto_Lead) { // The target has a rigidbody, and the "Use_Auto_Lead" option is enabled.
					// Calculate the lead angle to the target.
					float distance = Vector3.Distance (thisTransform.position, targetPosition);
					targetPosition += aimingScript.Target_Rigidbody.velocity * (distance / Bullet_Velocity);
				}
