...
namespace ChobiAssets.PTM
{
	...
	public class UI_Text_Control_CS : MonoBehaviour
	{
		...
		*/
		...
		void Update ()
		{
			if (textList.Count > 0 && isDisplaying == false) { // The list has any text informations, and this script is not displaying any text now.
		...
		IEnumerator Fade_In ()
		{
			...
			float targetAlpha = currentColor.a;
		...
		IEnumerator Fade_Out ()
		{
			...
			currentColor.a = 0.0f;
			thisText.color = currentColor;
			textList.RemoveAt (0); // Remove the text information from the list.
			isDisplaying = false;
		}
