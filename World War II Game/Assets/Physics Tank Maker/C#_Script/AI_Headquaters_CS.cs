...
namespace ChobiAssets.PTM
{
	...
	public class AI_Headquaters_CS : MonoBehaviour
	{
		...
		*/
		...
		List <AI_Headquaters_Helper_CS> friendlyTanksList = new List <AI_Headquaters_Helper_CS>();
		List <AI_Headquaters_Helper_CS> hostileTanksList = new List <AI_Headquaters_Helper_CS >();
		float orderCount;
		...
		void Assign_Target(List <AI_Headquaters_Helper_CS> teamA, List <AI_Headquaters_Helper_CS> teamB)
		{
			...
			for (int i = 0; i < teamA.Count; i++) {
				...
				AI_CS aiScript = teamA[i].AI_Script;
				if (aiScript == null || aiScript.Is_Sharing_Target || aiScript.Settings_Script.No_Attack || aiScript.Detect_Flag) { // The tank is not AI, or is sharing the target, or is set not to attack, or is detecting the current target.
					continue;
				}
				for (int j = 0; j < teamB.Count; j++) {
					if (teamB[j].Body_Transform.root.tag == "Finish") { // The target is alredy destroyed.
						continue;
					} // The target is living.

					// Find the closest enemy tank.
					float distance = Vector3.Distance(teamA[i].Body_Transform.position, teamB[j].Body_Transform.position);
					if (distance > aiScript.Settings_Script.Visibility_Radius || distance > shortestDistance) { // The target is out of the "Visibility_Radius", or is not the closest enemy.
						continue;
					} // The target is within the "Visibility_Radius", and is the closest enemy.

					// Check the AI can detect the target.
					if (aiScript.Check_for_Assigning(teamB[j]) == true) { // The target can be detected by the AI.
