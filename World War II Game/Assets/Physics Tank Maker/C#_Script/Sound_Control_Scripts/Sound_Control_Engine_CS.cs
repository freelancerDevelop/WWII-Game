	...
	public class Sound_Control_Engine_CS : MonoBehaviour
	{
		...
		*/
		...
		void Engine_Sound()
		{
			...
			// Set the rate.
			targetRate = (Left_Velocity + Right_Velocity) / 2.0f / Max_Velocity;
			if (targetRate > currentRate) { // Acceleration
				currentRate = Mathf.MoveTowards (currentRate, targetRate, accelerationRate * Time.fixedDeltaTime);
			}
			else { // Deceleration
				currentRate = Mathf.MoveTowards (currentRate, targetRate, decelerationRate * Time.fixedDeltaTime);
			}
