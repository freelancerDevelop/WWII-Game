...
namespace ChobiAssets.PTM
{
	...
	public class Event_Trigger_02_02_Destroy_AND_Necessary_CS : Event_Trigger_02_00_Destroy_Base_CS
	{
		...
		{
			if (allTriggersAreReady == false) { // All the trigger tanks have not been spawned yet.
				Observe_Trigger_Tanks();
			}

			if (spawnedTankCount >= eventControllerScript.Necessary_Num) { // The necessary number of trigger tanks have been spawned. 
				if (Check_Destroy_AND_Necessary(eventControllerScript)) { // The necessary number of trigger tanks have been destroyed.
		...
		{ // Check the necessary number of trigger tanks are destroyed.
			...
			for (int i = 0; i < triggerTanksList.Count; i++) {
				if (triggerTanksList[i].Root_Transform == null) { // The tank has already been removed from the scene.
					destroyedCount += 1;
					continue;
				}
				if (triggerTanksList[i].Root_Transform.tag == "Finish") { // The tank has been destroyed.
					destroyedCount += 1;
					if (triggerTanksList[i].Respawn_Script && triggerTanksList[i].Respawn_Script.Respawn_Times > 0) { // The "Respawn_Times" remains.
						destroyedCount -= 1;
					}
				}
			}
