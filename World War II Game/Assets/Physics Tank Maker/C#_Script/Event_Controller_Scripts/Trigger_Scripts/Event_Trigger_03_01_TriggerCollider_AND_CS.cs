	...
	public class Event_Trigger_03_01_TriggerCollider_AND_CS : Event_Trigger_03_00_TriggerCollider_Base_CS
	{
		...
		{ // Called from "Trigger_Collider_CS".
			...
			for (int i = 0; i < eventControllerScript.Trigger_Tanks.Length; i++) {
				...
				if (eventControllerScript.All_Trigger_Flag) { // Requires that all the triggers are detected.
					for (int j = 0; j < eventControllerScript.Trigger_Tanks.Length; j++) {
						if (eventControllerScript.Trigger_Tanks[j]) { // At least one of the trigger remains.
							return;
						}
					...
					if (detectedCount >= eventControllerScript.Necessary_Num) { // The necessary number of triggers are detected.
