...
namespace ChobiAssets.PTM
{
	...
	public class Event_Trigger_02_01_Destroy_AND_All_CS : Event_Trigger_02_00_Destroy_Base_CS
	{
		...
		public override void Check_Trigger()
		{
			...
			if (allTriggersAreReady) { // All the trigger tanks have been spawned.
				if (Check_Destroy_AND_All(eventControllerScript)) { // All the trigger tanks have been destroyed.
		...
		{ // Check all the trigger tanks are destroyed.
			for (int i = 0; i < triggerTanksList.Count; i++) {
				if (triggerTanksList[i].Root_Transform && triggerTanksList[i].Root_Transform.tag != "Finish") { // At least one of the trigger tanks is alive.
					return false;
				}
				else if (triggerTanksList[i].Respawn_Script && triggerTanksList[i].Respawn_Script.Respawn_Times > 0) { // The "Respawn_Times" remains.
					return false;
				}
			}
