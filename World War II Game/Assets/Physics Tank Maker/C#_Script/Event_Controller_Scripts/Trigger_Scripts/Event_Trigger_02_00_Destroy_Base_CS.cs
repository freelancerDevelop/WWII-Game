...
namespace ChobiAssets.PTM
{
	...
	public class Event_Trigger_02_00_Destroy_Base_CS : Event_Trigger_00_Base_CS
	{
		...
		protected void Observe_Trigger_Tanks()
		{ // Check the trigger tanks have been spawned or not.
			for (int i = 0; i < triggerTanksList.Count; i++) {
				if (triggerTanksList[i].Is_Registered) { // The tank has already been registered.
					continue;
				}

				// Check the trigger tank is exist.
				if (triggerTanksList[i].Root_Transform == null) { // The tank should have been removed from the scene.
