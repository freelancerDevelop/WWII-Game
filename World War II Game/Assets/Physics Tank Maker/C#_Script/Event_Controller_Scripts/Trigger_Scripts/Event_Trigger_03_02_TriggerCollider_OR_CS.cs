	...
	public class Event_Trigger_03_02_TriggerCollider_OR_CS : Event_Trigger_03_00_TriggerCollider_Base_CS
	{
		...
		{ // Called from "Trigger_Collider_CS".
			...
			for (int i = 0; i < eventControllerScript.Trigger_Tanks.Length; i++) {
				if (eventControllerScript.Trigger_Tanks[i] != detectedTransform) { // The "detectedTransform" is not the trigger.
					continue;
				}

				// The "detectedTransform" is one of the trigger.
				if (eventControllerScript.Trigger_Itself_Flag == true) { // The trigger should be the target of the events.
					switch (eventControllerScript.Event_Type) {
						...
						case 5: // "Destroy Tank"
							...
							if (eventControllerScript.Wait_For_All_Triggers == true) { // Keep this event until all the triggers touch the collider.
								...
								for (int j = 0; j < eventControllerScript.Trigger_Tanks.Length; j++) {
									if (eventControllerScript.Trigger_Tanks[j]) { // At least one of the triggers remains.
