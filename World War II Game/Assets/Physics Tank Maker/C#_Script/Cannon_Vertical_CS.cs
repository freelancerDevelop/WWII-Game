...
namespace ChobiAssets.PTM
{
	...
	public class Cannon_Vertical_CS : MonoBehaviour
	{
		...
		*/
		...
		bool useAutoElevationAngle;
		public float Bullet_Velocity = 250.0f; // Set by "Bullet_Generator_CS".
		...
		public void Start_Tracking (bool allowUsingAutoElevationAngle)
		{ // Called from "Aiming_Control_CS".
			isTracking = true;
			isTurning = true;
			useAutoElevationAngle = allowUsingAutoElevationAngle;
		}
		...
		void Auto_Turn ()
		{
			...
			float targetAngle;
			if (isTracking) { // Tracking the target.
				// Calculate the target angle.
				if (useAutoElevationAngle) {
					targetAngle = Auto_Elevation_Angle ();
				} else {
					targetAngle = Manual_Elevation_Angle ();
				}
				targetAngle += Mathf.DeltaAngle (0.0f, angleX) + aimingScript.Adjust_Angle.y;
			} else { // Not tracking. >> Return to the initial angle.
		...
		float Auto_Elevation_Angle ()
		{
			...
			float posBase = (Physics.gravity.y * Mathf.Pow (dist.x, 2.0f)) / (2.0f * Mathf.Pow (Bullet_Velocity, 2.0f));
			...
			else { // The bullet cannot reach the target.
				properAngle = 45.0f;
			}
