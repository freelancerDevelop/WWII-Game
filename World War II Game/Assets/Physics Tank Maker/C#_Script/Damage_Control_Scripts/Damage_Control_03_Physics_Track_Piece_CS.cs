...
namespace ChobiAssets.PTM
{
	...
	public class Damage_Control_03_Physics_Track_Piece_CS : Damage_Control_00_Base_CS
	{
		...
		{ // Called from "Bullet_Control_CS", when the bullet hits this collider.
			...
			if (centerScript.Receive_Damage(damage, 2, Track_Index) == true) { // type = 2 (Physics_Track piece), index = Track_Index (0 = Left, 1 = Right). true = The track has been destroyed.
		...
		{ // Called from "Damage_Control_Center_CS", when the track has broken.
			...
			// Change the hierarchy. (Get out of the hierarchy of the tank.)
