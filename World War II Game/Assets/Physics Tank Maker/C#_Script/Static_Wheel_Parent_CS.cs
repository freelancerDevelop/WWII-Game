	...
	public class Static_Wheel_Parent_CS : MonoBehaviour
	{
		...
		*/
		...
		{ // Called from "Track_Scroll_CS".
			if (tempScrollTrackScript.Is_Left) { // Left
				leftScrollTrackScript = tempScrollTrackScript;
				// Set the rotation rate.
				float referenceRadius = leftScrollTrackScript.Reference_Wheel.GetComponent <MeshFilter>().mesh.bounds.extents.x;
				scrollTrackRateL = referenceRadius / Wheel_Radius;
			}
			else { // Right
				rightScrollTrackScript = tempScrollTrackScript;
				// Set the rotation rate.
				float referenceRadius = rightScrollTrackScript.Reference_Wheel.GetComponent <MeshFilter>().mesh.bounds.extents.x;
		...
		void Update ()
		{
			...
			if (Is_Visible) {
				if (staticTrackScript && staticTrackScript.isActiveAndEnabled) { // (Note.) Static_Track is enabled and disbaled by "Track_LOD_Control_CS" in the runtime.
