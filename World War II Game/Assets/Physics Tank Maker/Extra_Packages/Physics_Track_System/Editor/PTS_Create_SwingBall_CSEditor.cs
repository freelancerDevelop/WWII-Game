	...
	public class PTS_Create_SwingBall_CSEditor : Editor
	{
		...
		void Find_RoadWheels ()
		{
			...
			foreach (PTS_Drive_Wheel_CS driveScript in driveScripts) {
				...
				if (wheelTransform.parent.GetComponent <PTS_Create_RoadWheel_CS> () && wheelTransform.GetComponent <MeshFilter> ()) { // RoadWheel && visible
		...
		void Find_SupportWheels ()
		{
			...
			foreach (PTS_Static_Wheel_CS wheelScript in wheelScripts) {
				...
				if (wheelTransform.parent.GetComponent <PTS_Create_SupportWheel_CS> ()) { // SupportWheel
					Vector3 localPos = thisTransform.InverseTransformPoint (wheelTransform.position);
					if (localPos.y > 0.0f) { // Left
						localPos.x += wheelTransform.GetComponent <SphereCollider> ().radius + RadiusProp.floatValue;
						posList.Add (localPos);
