...
namespace ChobiAssets.PTS
{
	...
	public class PTS_Track_Deform_CSEditor : Editor
	{
		...
		{
			if (thisGameObject && thisGameObject.transform.parent) {
				Set_Inspector ();
				if (GUI.changed) {
					Set_Vertices ();
				}
				...
				for (int i = 0; i < Anchor_ArrayProp.arraySize; i++) {
					...
					if (Anchor_ArrayProp.GetArrayElementAtIndex (i).objectReferenceValue != null) {
						...
						if (string.IsNullOrEmpty (temp_AnchorName) == false && string.IsNullOrEmpty (temp_AnchorParentName) == false) {
							Anchor_ArrayProp.GetArrayElementAtIndex (i).objectReferenceValue = thisGameObject.transform.parent.Find (temp_AnchorParentName + "/" + temp_AnchorName);
						}
					}
					EditorGUILayout.Space ();
		...
		void Set_Vertices ()
		{
			...
			// Set values.
			PTS_Track_Deform_CS deformScript = thisGameObject.GetComponent < PTS_Track_Deform_CS > ();
