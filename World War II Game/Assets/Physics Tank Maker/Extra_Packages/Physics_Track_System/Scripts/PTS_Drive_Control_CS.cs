...
namespace ChobiAssets.PTS
{
	...
	public class PTS_Drive_Control_CS : MonoBehaviour
	{
		...
		void Anti_Slip ()
		{
			...
			if (Physics.Raycast(ray, Ray_Distance, layerMask)) { // On the ground.
				Vector3 currentVelocity = bodyRigidbody.velocity;
				if (stopFlag) {
					currentVelocity.x *= 0.99f;
					currentVelocity.z *= 0.99f;
				} else {
					currentVelocity = Vector3.MoveTowards (currentVelocity, thisTransform.forward * (Mathf.Sign (vertical) * currentVelocity.magnitude), 50.0f * Time.fixedDeltaTime);
				}
